...
def barcode_scan(sequence, whitelist_dict, max_ed=2, min_ed_diff=1, flank=5, bc_length=10):
    ...
    if candidates and (len(candidates) == 1 or candidates[1][1] - candidates[0][1] >= min_ed_diff):
        best_match = candidates[0]
        if best_match[1] <= max_ed:
            return (whitelist_dict[best_match[0]], best_match[1], best_match[0], best_match[2])
...
def process_barcodes(uncorr_bc_file, output_dir, i7_whitelist, i5_whitelist, 
    ...
    with open(uncorr_bc_file, 'r') as input_file, open(corr_bc_file, 'w', newline='') as output_file:
        ...
        writer = csv.DictWriter(output_file, 
                                fieldnames=["read_id", "i7index_uncorr", "i7index_ed", "i7index_corr", "i7index_frameshift", \
                                    "i5index_uncorr", "i5index_ed", "i5index_corr", "i5index_frameshift", "wellid"], delimiter='\t')
        writer.writeheader()
